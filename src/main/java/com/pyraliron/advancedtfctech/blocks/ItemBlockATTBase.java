...
public class ItemBlockATTBase extends ItemBlock
{
    public ItemBlockATTBase(Block b)
    {
        super(b);
        if (((BlockATTBase) b).enumValues.length > 1)
            setHasSubtypes(true);
    }
    ...
    {
        return ((BlockATTBase) this.block).getUnlocalizedName(stack);
    }
    ...
    {
        if (((BlockATTBase) block).hasFlavour(stack))
        {
            String subName = ((BlockATTBase) this.block).getStateFromMeta(stack.getItemDamage()).getValue(((BlockIPBase) this.block).property).toString().toLowerCase(Locale.US);
            String flavourKey = "desc." + Reference.MOD_ID + ".flavor." + ((BlockATTBase) this.block).name + "." + subName;
