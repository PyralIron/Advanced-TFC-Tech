...
import static blusunrize.immersiveengineering.common.IEContent.fluidCreosote;
...
public class TileEntityPowerLoom extends TileEntityMultiblockMetal<TileEntityPowerLoom, IMultiblockRecipe> implements IAdvancedSelectionBounds, IAdvancedCollisionBounds, IGuiTile
{
    ...
    {
        super(MultiblockPowerLoom.instance, new int[]{2, 2, 3}, 16000, true);
    ...
    {
        return 0;//PumpjackHandler.getFluidAmount(world, getPos().getX() >> 4, getPos().getZ() >> 4);
    }

    public Fluid availableFluid()
    {
        return fluidCreosote;//PumpjackHandler.getFluid(world, getPos().getX() >> 4, getPos().getZ() >> 4);
    }

    public int getResidualOil()
    {
        return 100;//return PumpjackHandler.getResidualFluid(world, getPos().getX() >> 4, getPos().getZ() >> 4);
    }
    ...
    public void update(boolean consumePower)
    {
        ...
        int consumed = 1;//IPConfig.Extraction.pumpjack_consumption;
        ...
        {
            if ((getPos().getX() + getPos().getZ()) % 10 == 0)//IPConfig.Extraction.pipe_check_ticks == pipeTicks)
            {
            ...
            if (lastHadPipes)
            {
                ...
                if (availableOil() > 0 || residual > 0)
                {
                    ...
                    active = true;
                    FluidStack out = new FluidStack(availableFluid(), Math.min(/*IPConfig.Extraction.pumpjack_speed*/10, oilAmnt));
            ...
            pipeTicks = (pipeTicks + 1) % 10;//IPConfig.Extraction.pipe_check_ticks;
        }
    ...
    public List<AxisAlignedBB> getAdvancedSelectionBounds()
    {
        ...
        else if (pos == 18 || this.field_174879_c > 0)
        {
